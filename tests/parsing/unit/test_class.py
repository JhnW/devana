...
class TestClassTemplate(unittest.TestCase):
    ...
    def test_basic_concept_class(self):
        node = find_by_name(self.cursor, "BasicConceptClass")
        result = ClassInfo.from_cursor(node)
        self.assertTrue(result.is_class)
        self.assertEqual(result.name, "BasicConceptClass")
        ...
        method: MethodInfo = cast(MethodInfo, result.private[0])
    ...
    def test_concept_class(self):
        ...
        self.assertEqual(method.arguments[0].type.is_generic, True)
        self.assertEqual(method.arguments[0].type.details.name, "T")
        self.assertEqual(len(method.requires), 1)
        self.assertEqual(method.requires[0].name, "TestConcept")
        self.assertEqual(method.requires[0].body, "true")
        self.assertEqual(method.requires[0].is_requirement, True)
    ...
    def test_concept_struct(self):
        ...
        self.assertEqual(result.template.requires[0:4], ["false", "or", "true", "and"])
        self.assertEqual(result.template.requires[4].name, "TestConcept")
        self.assertEqual(result.template.requires[4].is_requirement, True)
        self.assertEqual(len(result.template.parameters), 2)
        ...
        self.assertEqual(method.body, None)
        self.assertEqual(len(method.requires), 5)
        self.assertEqual(method.requires[0], "(")
        self.assertEqual(method.requires[1].name, "TestConcept")
        self.assertEqual(method.requires[1].is_requirement, True)
        self.assertEqual(method.requires[1].body, "true")
        self.assertEqual(method.requires[2:5], ["||", "true", ")"])
